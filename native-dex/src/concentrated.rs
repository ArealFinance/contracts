...
// CP-5 — `distribute_to_bins` removed. The Monotonic Ladder rewrite
// rejects user LP on master pools with `MasterPoolUserLpDisabled`
// (`add_liquidity` / `zap_liquidity`); the Nexus path uses
// `grow_liquidity` / `compress_liquidity` (CP-7) to size the active
// bid wall instead of uniform/proportional distribution. The original
// helper body lived between CP-3 and CP-5 and is preserved in git
// history at commit e42072e (CP-4 snapshot).
