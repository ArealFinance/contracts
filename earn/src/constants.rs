...
// L2: hard ceiling for the tunable mint fee. The fee is pure DAO revenue charged
// on top of the deposit body; capping it at 10% prevents the authority from
// silently confiscating most of a deposit (the unbounded BPS_DENOMINATOR / 100%
// allowance would let the fee equal the entire body).
pub const MAX_MINT_FEE_BPS: u16 = 1_000; // 10% ceiling

pub const MIN_MINT_AMOUNT: u64 = 1_000_000; // $1.00 minimum deposit (anti-dust)

// L3: hard ceiling for the tunable anti-dust floor (USDC base units, 6-dec).
// $1,000,000 is far above any legitimate dust floor; capping here stops the
