...
//!
//! Layer 8 Step 1 — interface stubs only. The body is filled in Step 4
//! (`claim_yield`). RWT Engine's vault PDA signs the YD::claim CPI as the
//! claimant; the crank acts as payer.
//!
...
/// CPI → `yield_distribution::claim` signed by the RWT vault PDA.
///
/// Account order MUST match
/// `contracts/yield-distribution/src/instructions/claim.rs` (10 accounts):
///   0. claimant         (signer via PDA)  — `["rwt_vault", ot_mint]`
...
///   7. claimant_token   (mut)             — claimant's RWT ATA
///   8. token_program
///   9. system_program
///
...
#[allow(clippy::too_many_arguments, dead_code)] // dead_code: stub, wired up in Step 4
pub fn cpi_yd_claim<'a>(
    ...
    // TODO(Layer 8 Step 4): build instruction data + accounts array,
    // then invoke_signed::<11> with the RWT vault PDA seeds
    // (`["rwt_vault", ot_mint]`).
    // See `Layer 8 architecture §3.2` and `§5.2.3`.
    unimplemented!("cpi_yd_claim is a Layer 8 Step 1 stub; implement in Step 4")
}
